...
from deep_translator.constants import BASE_URLS, WIKIMEDIA_ENV_VAR
...
class WikimediaTranslator(BaseTranslator):
    """
    ...
    def __init__(
        ...
        source: str = "en",
        target: str = "de",
        api_key: Optional[str] = os.getenv(WIKIMEDIA_ENV_VAR, None),
        **kwargs
    ):
        """
        @param api_key: your wikimeida api key
        """
        if not api_key:
            raise ApiKeyException(WIKIMEDIA_ENV_VAR)
        ...
        super().__init__(
            base_url=BASE_URLS.get("WIKIMEDIA"),
            source=source,
            target=target,
            **kwargs
        )
